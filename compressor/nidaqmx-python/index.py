...
from nidaqmx_examples.CiSwController import CiSwController
...
def createController():
    ...
    for slotInfo in config.get('slots'):
        ...
        if model == '9201':    # 전압 ai_voltage_sw_timed 모듈을 실행
            pp.pprint('9201 Start')
            controller = AiVoltageSwController(config, slotInfo)
        if model == '9208':    # 전류 ai_voltage_sw_timed 모듈을 실행
            pp.pprint('9208 Start')
            controller = CiSwController(config, slotInfo)
        elif model == '9482':  # 릴레이 do_sw_timed 모듈 실행
            pp.pprint('9482 Start')
