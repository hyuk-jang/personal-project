...
router.get('/login', (req, res) => {
  //   BU.CLI('Get Login');
  const projectSourceInfo = commonUtil.convertProjectSource(
    config.projectInfo.projectMainId,
  );

  if (config.dev.isAutoAuth === 1) {
    // BU.CLI('자동 로그인', config.dev);
    // global.app.set('auth', true);
    if (!req.user) {
      // BU.CLI('poost!');
      const urlPort = config.webServer.httpPort;
      request.post(
        {
